...
The elixir library [GenStage](https://hexdocs.pm/gen_stage/GenStage.html) does exactly that, and is the tool we will use to solve this problem.

## Exploring GenStage

GenStage is an Elixir behaviour for exchanging events with back-pressure between Elixir processes, called `stages`.
Stages are the basic building blocks for creating data processing pipelines.
...
A stage may be both a producer and a cosumer at the same time, acting as a `consumer producer`.
...
We can see that the producer code looks very similar to a [GenServer](https://hexdocs.pm/elixir/1.12/GenServer.html), 
...
```bash
[info] PageProducer init
[info] PageConsumerSupervisor init
[info] Received demand for 1 pages
[info] PageConsumer received "https://minecraft.wiki/"
[info] Scraped page with title The Minecraft Wiki
[info] PageConsumer received "https://minecraft.wiki/w/Minecraft_Wiki:Community_portal"
[info] PageConsumer received "https://minecraft.wiki/w/Minecraft_Wiki:Forum"
[info] Scraped page with title Minecraft Wiki:Community portal – Minecraft Wiki
[info] PageConsumer received "https://minecraft.wiki/w/Minecraft_Wiki:Admin_noticeboard"
```
...
avoid making more network calls, but specialized message queues are still much faster and robust. Message
queues also allow more versatility, like using different programming languages, dead letter queue strategies
